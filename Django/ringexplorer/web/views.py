...
def getRing(request):
	...
	data = ring.objects.filter(ringindex = query).annotate(rate1=F('order1amount') / F('order1nextamount')).annotate(rate2=F('order2amount') / F('order2nextamount')).annotate(rate3=F('order3amount') / F('order3nextamount'))
