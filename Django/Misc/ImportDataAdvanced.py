...
for i, row in stored_data.iterrows():
    ...
    command = command + " \nvalues(" + str(row['ringsize']) +", " + str(row['dataset']['ringIndex']) +", "+ str(row['block']) +", "+ str(row['timestamp']) +", Ethereum, " + str(row['dataset']['ringHash']) +", " + str(row['dataset']['miner']) +", "+ str(row['dataset']['miner']) +", "
    print(command)
    #c.execute(command)
#conn.close()
